...
async function run() {
  try {
    ...
    // Remove legacy misconfigured Product Types
    await client.query(`
      DELETE FROM ${schema}.frs9_param_commond WHERE param_code = 'B0016' AND value2 = 'PRODUCT_TYPE';
    `);

    // Ensure headers exist just in case
    await client.query(`
      INSERT INTO ${schema}.frs9_param_commonh (param_code, param_name, param_usage, param_type, is_active, requires_approval, createdby, createddate, createdhost) 
      VALUES
