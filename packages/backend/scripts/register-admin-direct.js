...
/**
 * Direct Admin Registration Script
...
async function main() {
  try {
    console.log('🚀 Starting direct admin registration...');
      ...
      if (existingUser.rows.length > 0) {
        ...
        await platformClient.query(
          `UPDATE platform_admin.users
           ...
           WHERE email = $4`,
          [passwordHash, 'IAF Administrator', 'PLATFORM_ADMIN', ADMIN_EMAIL]
        );
        ...
        const result = await platformClient.query(
          ...
          ['admin', ADMIN_EMAIL, passwordHash, 'IAF Administrator', 'PLATFORM_ADMIN', true, null]
        );
      ...
      if (existingTenantUser.rows.length > 0) {
        ...
        await tenantClient.query(
          `UPDATE core.users
           ...
           WHERE email = $3`,
          [passwordHash, 'IAF Administrator', ADMIN_EMAIL]
        );
        ...
        const insertValues = ['admin', ADMIN_EMAIL, passwordHash, 'IAF Administrator'].slice(0, insertColumns.length);
      ...
      if (platformUser?.id) {
        await platformClient.query(
          `INSERT INTO platform_audit.global_audit_log
           ...
           VALUES ($1, $2, $3, $4, NOW())`,
          [platformUser.id, 'USER_MANAGEMENT', 'USER_CREATED', 'Admin user created: ' + ADMIN_EMAIL]
        );
      }
