...
class IFRS9Server {
  ...
  private setupMiddleware(): void {
    ...
    // 🔧 SURGICAL FIX: Cloudflare CORS bypass middleware (BEFORE CORS)
    // This handles the case where Cloudflare strips Origin headers
    const cloudflareCorsBypassMiddleware = require('./middleware/cloudflare-cors-bypass').cloudflareCorsBypassMiddleware;
    this.app.use(cloudflareCorsBypassMiddleware);
    console.log('✅ Cloudflare CORS bypass middleware configured');
    ...
    // 🚀 NEW: ULTIMATE CORS BYPASS for Cloudflare header stripping
    // This is the most comprehensive CORS handler that ensures communication works
    const { ultimateCorsBypass, megaCorsBypass } = require('./middleware/cors');

    // Use environment-based CORS strategy
    if (process.env.NODE_ENV === 'development' || process.env.CORS_BYPASS_MODE === 'mega') {
      // Development: Use mega bypass (allows everything)
      this.app.use(megaCorsBypass);
      console.log('🔥 MEGA CORS BYPASS: All origins allowed (development mode)');
    } else {
      // Production: Use intelligent bypass
      this.app.use(ultimateCorsBypass);
      console.log('🚀 ULTIMATE CORS BYPASS: Intelligent origin detection configured');
    }

    // ✅ ENHANCED CORS: Add manual CORS handler to ensure headers are always set
    // This handles Cloudflare header stripping and ensures CORS compliance
    const { handleCors } = require('./middleware/cors');
    this.app.use(handleCors);
    console.log('✅ Enhanced CORS handler configured for Cloudflare compatibility');

    // Compression
    this.app.use(compression());

    // Rate limiting (now with proper proxy trust and development awareness)
    if (config.features.apiRateLimit) {
      ...
      this.app.use('/api/', rateLimit({
        ...
        keyGenerator: (req) => {
          return req.ip || 'unknown'; // This will now correctly use X-Forwarded-For when available
        },
        // Skip successful requests from counting (reduces false rate limits)
        skipSuccessfulRequests: false
      }));
      console.log(`✅ Rate limiting configured (Development: ${isDevelopment ? '5000' : '1000'} requests/15min)`);
    }
