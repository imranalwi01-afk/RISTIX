...
# Print authentication configuration on startup (only in debug mode)
print_auth_config <- function() {
  # Silent printing to prevent log corruption
...
get_dev_user_context <- function() {
  ...
  # Debug output controlled by R_ANALYTICS_DEBUG_MODE
  if (Sys.getenv("R_ANALYTICS_DEBUG_MODE", "false") == "true") {
...
validate_user_context <- function(user_context) {
  ...
  if (auth_config$require_user_id && is.null(user_context$user_id)) {
    if (auth_config$log_auth_attempts && Sys.getenv("R_ANALYTICS_DEBUG_MODE", "false") == "true") {
