# =============================================================================
# CENTRALIZED CONFIGURATION SYSTEM - IAF R ANALYTICS (FIXED VERSION)
# =============================================================================
...
CentralizedConfiguration <- R6Class("CentralizedConfiguration",
  ...
  private = list(
    ...
    # Detect deployment environment (WITHOUT curl warnings)
    detect_environment = function() {
      # Priority 1: Check explicit environment variable
      deployment_target <- Sys.getenv("DEPLOYMENT_TARGET", "")
      if (deployment_target != "" && deployment_target %in% c("localdev", "iafecs")) {
        return(deployment_target)
      }

      # Priority 2: Check for IAF-specific patterns in hostname (no curl)
      hostname <- Sys.getenv("HOSTNAME", "")
      if (grepl("iaf|ecs|alibaba|10\\.18", hostname, ignore.case = TRUE)) {
        return("iafecs")
      }

      # Priority 3: Check NODE_ENV=production with IAF patterns
      node_env <- Sys.getenv("NODE_ENV", "")
      if (node_env == "production" && grepl("iaf", hostname, ignore.case = TRUE)) {
        return("iafecs")
      }

      # Priority 4: Check for ECS indicators without curl
      if (file.exists("/etc/ecs/ecs.config") ||
          Sys.getenv("ECS_CONTAINER_METADATA_URI") != "" ||
          Sys.getenv("AWS_EXECUTION_ENV") == "ECS") {
        return("iafecs")
      }
    ...
    load_production_config = function() {
      ...
      return(list(
        ...
        urls = list(
          ...
          backend_api = Sys.getenv("BACKEND_API_URL", "https://ristix.bdo-ki.com/api/v1"),
    ...
    load_development_config = function() {
      ...
      return(list(
        ...
        security = list(
          ...
          session_timeout = as.integer(Sys.getenv("SESSION_TIMEOUT", "28800")),
          enable_auth = as.logical(Sys.getenv("ENABLE_AUTH", "false")),  # Default: false for development
          auth_backend_url = Sys.getenv("AUTH_BACKEND_URL", "https://bifrs9-iaf.ifrspro.id"),
          auth_timeout = as.integer(Sys.getenv("AUTH_TIMEOUT", "30"))  # API timeout in seconds
        ),
        features = list(
  ...
  public = list(
    ...
    initialize = function() {
      if (is.null(private$config_cache)) {
        # Determine environment here instead of in a separate method
        deployment_target <- Sys.getenv("DEPLOYMENT_TARGET", "NOT_SET")

        if (deployment_target == "NOT_SET") {
          deployment_target <- private$detect_environment()
        }

        # Silent mode: No environment detection output to prevent log corruption

        if (deployment_target == "iafecs") {
          private$config_cache <- private$load_production_config()
        } else {
          private$config_cache <- private$load_development_config()
        }
      }
...
# Global Configuration Instance (backward compatibility) - Silent initialization
config_manager <- local({
  if (Sys.getenv("R_ANALYTICS_DEBUG_MODE", "false") == "true") {
    cat("🔧 Initializing centralized configuration manager...\n")
  }
  get_config_manager()
})