// packages/frontend/src/components/parameters/product/ProductParameterCreate.tsx
// ============================================================================
// 🔧 PROD-005A: PRODUCT PARAMETER CREATE - REACT ADMIN COMPONENT
// ============================================================================
// ✅ IMPLEMENTS: React Admin Create form for Product Parameters
// ✅ PATTERN: React Admin v4 Create with Material-UI v6 integration
// ✅ FEATURES: Form validation, conditional fields, save/save and add another
...
const validateProductCode = [
  required(),
  regex(/^[A-Z0-9_-]+$/, 'Product code must contain only uppercase letters, numbers, underscores, and hyphens'),
  minValue(1),
  maxValue(20)
];
...
const ProductParameterCreateToolbar = () => {
  const notify = useNotify();
  const redirect = useRedirect();

  const handleSaveAndAdd = () => {
    notify('Product parameter created successfully');
    redirect('create', 'product-parameters');
  };
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} sm={6}>
          <TextInput
            ...
            helperText="Unique identifier (uppercase letters, numbers, underscores, hyphens only)"
            fullWidth
          />
        </Grid>
        <Grid item xs={12} sm={6}>
...
const InformationAlert = () => (
  <Alert severity="info" sx={{ mb: 3 }}>
    <Typography variant="subtitle2" gutterBottom>
      ...
      Product Parameter Guidelines
    </Typography>
    <Stack spacing={1}>
      <Typography variant="body2">
        • Product Code must be unique and contain only uppercase letters, numbers, underscores, and hyphens
      </Typography>
      <Typography variant="body2">
        • Expected Life should reflect the typical duration of the banking product
      </Typography>
      <Typography variant="body2">
        • Borrowing and Market Rates are used for IFRS 9 calculations and should be entered as percentages
      </Typography>
      <Typography variant="body2">
        • Configuration flags control how this product behaves in various banking calculations
      </Typography>
    </Stack>
  </Alert>
);
...
export const ProductParameterCreate = () => {
  ...
  useEffect(() => {
    const fetchInstrumentClassOptions = async () => {
      try {
        console.log('🔍 [PROD-005A] Fetching instrument class options from API');
        setLoading(true);
        setError(null);
        ...
        if (result.success && result.data) {
          console.log('✅ [PROD-005A] Instrument class options loaded:', result.data);
          setInstrumentClassOptions(result.data);
        } else {
          throw new Error(result.message || 'Failed to fetch instrument class options');
        }
      } catch (err) {
        console.error('❌ [PROD-005A] Error fetching instrument class options:', err);
  ...
  if (loading) {
    return (
      <Create title="Create Product Parameter">
        <Loading />
      </Create>
    );
  }
  ...
  >
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 2 }}>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
        Add a new banking product parameter for IFRS 9 calculations and risk management.
      </Typography>
      
      <Divider sx={{ mb: 3 }} />
      
      <SimpleForm toolbar={<ProductParameterCreateToolbar />}>
        <InformationAlert />
...
export default ProductParameterCreate;

console.log('✅ [PROD-005A] ProductParameterCreate component loaded - React Admin Create form with Material-UI v6');