...
export default function ReviewScenarioPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await individualImpairmentAPI.getScenarios();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await individualImpairmentAPI.createScenario(payload);
      setDialogOpen(false);
      loadData();
    } catch (e: any) {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'description', headerName: 'Description', flex: 1 },
    {
      ...
      width: 100,
      valueGetter: (value, row) => row?.configuration?.nScenarios || '-'
    },
    {
      field: 'status',
      headerName: 'Status',
