...
export const businessSetupRoutes = new Hono()
...
// GET /banking/setup/business - List all business settings
businessSetupRoutes.get('/', async (c) => {
    try {
        const settings = await db.query.frs9ParamCommonh.findMany({
            where: eq(frs9ParamCommonh.paramType, 'B'),
            with: {
                details: true
            }
        });
...
// GET /banking/setup/business/:code - Get specific setting
businessSetupRoutes.get('/:code', async (c) => {
...
// POST /banking/setup/business - Create new setting header
businessSetupRoutes.post('/', zValidator('json', settingSchema), async (c) => {
    const data = c.req.valid('json');
    data.paramType = 'B'; // Force types B
    try {
...
// PUT /banking/setup/business/:code - Update setting header
businessSetupRoutes.put('/:code', zValidator('json', settingSchema.partial()), async (c) => {
    const code = c.req.param('code');
    const data = c.req.valid('json');
...
// POST /banking/setup/business/details - Create/Add detail to a setting
businessSetupRoutes.post('/details', zValidator('json', detailSchema), async (c) => {
    const data = c.req.valid('json');
    try {
...
// DELETE /banking/setup/business/details/:id
businessSetupRoutes.delete('/details/:id', async (c) => {
