-- Migration: Normalize legacy system control flags to binary strings ('1' / '0')
-- Scope: Legacy FRS9 parameter tables (public/ifrs9.frs9_param_commonh + frs9_param_commond)
-- Reason: Stored procedures read S1003/S1006 from param_usage and fail when it contains label text.
...
BEGIN
    ...
    ELSIF to_regclass('ifrs9.frs9_param_commonh') IS NOT NULL
       AND to_regclass('ifrs9.frs9_param_commond') IS NOT NULL THEN
        v_schema := 'ifrs9';
    ELSE
        RAISE NOTICE 'Skipping 0025: frs9_param_commonh/frs9_param_commond not found';
        RETURN;
    END IF;

    -- Ensure known procedure system flags exist with machine-readable defaults.
    EXECUTE format($sql$
        ...
        VALUES
            ('S1003', 'IS_PAYM_UPLOAD', '1', 'S', 'SYSTEM', CURRENT_DATE, 'migration'),
            ('S1004', 'INTERVAL_MODEL_RUN', '1', 'S', 'SYSTEM', CURRENT_DATE, 'migration'),
            ('S1006', 'WO INCREMENT', '0', 'S', 'SYSTEM', CURRENT_DATE, 'migration')
    ...
    EXECUTE format($sql$
        ...
        VALUES
            ('S1003', 1, '1', 'Enabled',  'Y',   'Enable Payment Upload',               'SYSTEM', CURRENT_DATE, 'migration'),
            ('S1003', 2, '0', 'Disabled', 'N',   'Disable Payment Upload',              'SYSTEM', CURRENT_DATE, 'migration'),
            ('S1004', 1, 'Daily',   '1',  'DAILY',   'Run model daily',                 'SYSTEM', CURRENT_DATE, 'migration'),
            ('S1004', 2, 'Weekly',  '7',  'WEEKLY',  'Run model weekly',                'SYSTEM', CURRENT_DATE, 'migration'),
            ('S1004', 3, 'Monthly', '30', 'MONTHLY', 'Run model monthly',               'SYSTEM', CURRENT_DATE, 'migration'),
            ('S1006', 1, '0', 'Disabled', 'NO',  'Write Off increment disabled',        'SYSTEM', CURRENT_DATE, 'migration'),
            ('S1006', 2, '1', 'Enabled',  'YES', 'Write Off increment enabled',         'SYSTEM', CURRENT_DATE, 'migration')
    ...
    -- Normalize all system params that are modeled as binary options in detail rows.
    EXECUTE format($sql$
    ...
    -- Normalize numeric interval flag used by model-sequence procedures.
    EXECUTE format($sql$
