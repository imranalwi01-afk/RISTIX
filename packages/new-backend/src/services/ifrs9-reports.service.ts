...
export class Ifrs9ReportsService {
    ...
    async getLifetimeLGDDetail(tenantId: string, page: number, limit: number, params?: LifetimeLGDParams) {
        try {
            ...
            // Matching SQL script logic:
            // SELECT A.ACCOUNT_NUMBER, A.CIF_NAME, B.PRC_DATE AS FIRST_NPL_DATE, B.EQV_AT_DEFAULT AS OS_AT_DEFAULT, C.SEQ, C.NPV_EQV_REC AS PV_RECOVERY
