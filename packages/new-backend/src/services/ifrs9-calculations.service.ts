...
export class Ifrs9CalculationsService {
    ...
    async getSummary(tenantId: string, requestedDate?: string, mode?: string): Promise<DashboardSummaryPayload> {
        try {
            ...
            const isAllPeriods = requestedDate === 'all';

            try {
                const conditions = [];
                if (!isAllPeriods && prcDate) {
                    conditions.push(eq(frs9EclSummary.prcDate, prcDate as any));
                }
                if (hasSegmentFilter) {
                ...
                if (row && Number(row.totalAccounts) > 0) {
                    ...
                    return {
                        data: {
                            ...
                            lastUpdated: isAllPeriods ? 'Cumulative Grand Total (All Periods)' : (prcDate || 'No data'),
                            currency: 'IDR',
                            isFallback: false,
                        },
            ...
            return {
                data: {
                    ...
                    impairedRatio: 0,
                    coverageRatio: 0,
                    lastUpdated: prcDate || requestedDate || 'No data',
                    currency: 'IDR',
                    isFallback: false,
                },
                debug: this.buildSummaryDebug(
                    ...
                    mode,
                    segmentIds,
                    `select sum(ecl_final_amt) as total_ecl, sum(outstanding) as total_exposure, sum(noa) as total_accounts, sum(case when stage = '3' then ecl_final_amt else 0 end) as stage3_ecl from public.frs9_ecl_summary${!isAllPeriods && prcDate ? ' where prc_date = :effectiveDate' : ''}${hasSegmentFilter ? `${!isAllPeriods && prcDate ? ' and' : (!isAllPeriods ? ' where' : ' where')} segment_id in (...)` : ''}`,
                    ['No rows were found in frs9_ecl_summary for the current filter scope.'],
                ),
            };
        } catch (error: any) {
            ...
            if (this.isLegacySchemaError(error)) {
                return {
                    ...
                    debug: this.buildSummaryDebug(
                        ...
                        mode,
                        [],
                        'Legacy schema error prevented frs9_ecl_summary query resolution.',
                        ['No summary rows were returned because frs9_ecl_summary could not be queried.'],
                    ),
                };
            }
    ...
    async getPortfolioTrend(tenantId: string, endDate?: string, _mode?: string): Promise<DashboardTrendPayload> {
        try {
            ...
            // 1. Query summary table only - aggregated ECL by stage over time
            let trend: any[] = []
            ...
            if (!trend || trend.length === 0) {
                return {
                    ...
                    debug: this.buildTrendDebug(
                        ...
                        segmentIds,
                        sqlPreview,
                        ['No trend rows were found in frs9_ecl_summary for the current filter scope.'],
                    ),
                };
            }
            ...
            if (this.isLegacySchemaError(error)) {
                return {
                    ...
                    debug: this.buildTrendDebug(
                        ...
                        _mode,
                        [],
                        'Legacy schema error prevented frs9_ecl_summary trend query resolution.',
                        ['No trend rows were returned because frs9_ecl_summary could not be queried.'],
                    ),
                };
            }
    ...
    async getAvailableDates(tenantId: string, mode?: string, groupBy?: string) {
        try {
            console.log(`📅 Fetching available dashboard dates from frs9_ecl_summary [Mode: ${mode || 'all'}]...`);
            ...
            // Collect all unique dates from summary only
            ...
            console.log(`✅ Dashboard available dates: ${sortedDates.length} unique dates found in frs9_ecl_summary`);
