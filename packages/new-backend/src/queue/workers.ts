...
export function setupApprovalNotificationWorker(db: PostgresJsDatabase<typeof schema>) {
    const worker = new Worker<ApprovalNotificationJob>(
        ...
        { connection: connectionOptions as any, concurrency: 5, lockDuration: 60000, maxStalledCount: 1, stalledInterval: 30000 }
    )
...
export function setupECLCalculationWorker(db: PostgresJsDatabase<typeof schema>) {
    const worker = new Worker<ECLCalculationJob>(
        ...
        { connection: connectionOptions as any, concurrency: 2, lockDuration: 7200000, maxStalledCount: 0, stalledInterval: 300000 } // Lower concurrency for heavy calculations
    )
