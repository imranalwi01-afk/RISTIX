---
title: Banking User Guides
description: Operational guides for banking setup, approval, audit, and IFRS 9 workflows.
sidebar_position: 4
---

# Banking User Guides

Use this section for operational tasks inside the banking application.

## Start by Intent

- `I need to submit or edit data`: start with [Setup and Parameter Guides](./setup) or [Collective Setup Guides](./collective)
- `I need to approve or review a request`: start with [Approval Inbox Guide](./approval/approval-inbox)
- `I need to trace what happened`: start with [Audit and Request Trace Guide](./audit/audit-and-request-trace)
- `I need to run a report`: start with [IFRS 9 Report Guides](./reports)
- `I need a complete end-to-end walk-through`: start with [End-to-End Scenarios](../scenarios)

## Key Pages

- [Submit a Setup Change for Approval](./setup/submit-setup-change)
- [Setup and Parameter Guides](./setup)
- [Collective Setup Guides](./collective)
- [IFRS 9 Report Guides](./reports)
- [Approval Inbox Guide](./approval/approval-inbox)
- [Audit and Request Trace Guide](./audit/audit-and-request-trace)
- [End-to-End Scenarios](../scenarios)
- [IAF User Role Matrix](./iaf-user-role-matrix)
- [Banking Sidebar Sitemap](./sidebar-sitemap)

## Common Business Journeys

- [LGD Setup Change to Approval and Audit Trace](../scenarios/lgd-change-approval-trace)
- [Product Parameter Change Lifecycle](../scenarios/product-parameter-approval-cycle)
- [IFRS 9 Report Validation and Export Flow](../scenarios/report-validation-and-export)

## Formal Testing Support

- [QA and UAT Packs](../qa-uat)
- [QA/UAT Execution Pack Template](../qa-uat/uat-execution-pack-template)
